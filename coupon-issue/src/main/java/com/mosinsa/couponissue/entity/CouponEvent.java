package com.mosinsa.couponissue.entity;
...
public class CouponEvent {
    ...
    public Coupon issuanceCoupon(Long customerId) {

        Assert.isTrue(quantity>0,"수량이 부족합니다.");
        Assert.isTrue(coupons.size()>0,"수량이 부족합니다.");
        Assert.isTrue(customerCouponEvent.add(new CustomerCouponEvent(customerId, this)),"이미 참여한 이벤트입니다.");

        Coupon coupon = this.getCoupons().get(--this.quantity);
        coupon.issuanceCouponToCustomer(customerId);
